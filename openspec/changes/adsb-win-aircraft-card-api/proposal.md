...
- Drops the previously forward-plumbed `xpProgressToNextTier`/"N% to next tier" progress bar: the real API gives a current `xp` count and a current `tier` name (e.g. "Alloy") but no documented XP-per-tier threshold to compute a percentage from, and that `tier` axis is adsb.win's own distinct *material*-tier ladder — unrelated to and not replacing this app's existing *rarity*-tier ladder (`aircraft-rarity` capability, Decision 5), which stays wired to its own local vendored dataset, untouched by this change. See design.md Decision 4.
