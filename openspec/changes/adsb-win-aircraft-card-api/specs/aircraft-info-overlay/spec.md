...
- **Successful card result**: a stat grid showing unique registrations, flights captured, observed flight time, and highest altitude observed (rendering an explicit placeholder for highest altitude when the API reports it as `null`), plus the account's current XP count, current adsb.win tier name, and a progress-to-next-tier bar computed from a provisional, explicitly-approximate default XP threshold table (no per-tier threshold is documented by adsb.win's API; see `adsb-win-aircraft-card-api`'s design.md Decision 4/4a) — never claiming 100%/promotion on a non-max tier ahead of what the API's own `tier` field reports, and rendering no bar at all (XP count and tier name only) if the reported tier name isn't one this table recognizes. This tier name and progress bar are adsb.win's own account-progression tier and are displayed separately from, and do not affect, `PlaneCard`'s existing `rarityTier`-driven frame/badge styling (`aircraft-rarity` capability), which continues to be sourced independently and is unaffected by this requirement.
