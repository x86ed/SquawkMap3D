...
- **Inverted fill per theme, not a single shared color.** A dark, semi-transparent fill (deep navy) darkens the night side correctly against the light basemap, but that same dark fill is nearly invisible layered over the already-dark dark-theme basemap. Caught immediately in manual testing, not assumed away: the dark theme uses a light fill (`#e8f2ff`) instead, so the night side reads as *brighter* than its surroundings there — same shading logic (same bands, same opacity, same polygons), just an inverted color per theme, mirroring `AIRPORT_FILL_COLOR`'s existing `Record<MapTheme, string>` pattern in `layers.ts`. Since `addTerminatorLayers` already re-runs on every `style.load` (a theme swap discards and re-adds all custom layers, same as every other layer here), threading `theme` through it and picking the color there was a small, natural extension of the existing architecture — no separate live-recolor path needed.
