...
MapLibre's `symbol`/`circle`/`line` layers drape onto the ground or terrain surface; there's no per-feature "float this marker N meters above the terrain" property. Placing an aircraft icon at its real barometric altitude — the core of the aeris look — needs a renderer with per-feature elevation. `@deck.gl/mapbox`'s `MapboxOverlay` in `interleaved: true` mode is the standard way to composite deck.gl layers into an existing MapLibre `Map` instance (shared WebGL context, correct depth-sorting against terrain/3D buildings). `IconLayer` (`getElevation`) renders the aircraft icons; `PathLayer` (`getElevation` per path vertex) renders the altitude-colored trails.
...
1. Look up `aircraft.t` (ICAO type designator, e.g. `A320`) directly against vendored `public/aircraft-shapes/<TYPE>.svg` (AircraftShapesSVG's filenames are ICAO type designators, confirmed against the repo listing — e.g. `A320.svg`, `A20N.svg`, `B738.svg`).
2. If no file matches, map `aircraft.category` (ADS-B emitter category, `A0`-`D7`) to one of pw-silhouettes' `generics/` groups (`heavy`, `light`, `medium1`/`medium2`, `high_vortex`, `high_performance`, `rotorcraft`, `glider_sailplane`, `lighter_than_air`, `ultralight_hangglider_paraglider`, `parachutist_skydiver`, `unmanned_aerial_vehicle`, `space_or_transatmospheric`, plus the two surface-vehicle groups for ground traffic) via a static lookup table in `constants.ts`, and use `public/aircraft-silhouettes/generics/<group>.svg`.
...
- **[Risk] New major dependency (deck.gl family)**: first non-MapLibre rendering dependency in the app; bundle size and an additional WebGL-integration surface to maintain. → *Mitigation*: `@deck.gl/mobxox` overlay is dynamically imported client-side only (matching how `MapView.tsx` already excludes MapLibre from SSR), keeping it out of any server bundle; scope strictly to `IconLayer`/`PathLayer`, not deck.gl's wider layer catalog.
