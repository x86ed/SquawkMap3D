...
- Add click-to-open popup on airport icons showing: IATA code, ICAO code, name, country flag, city, and country name.
...
- `airports-layer`: airports now render via a themed SVG icon (not a plain circle), are shown/hidden via a dedicated toggle control, and are clickable to open a popup with IATA/ICAO code, name, flag, city, and country.
...
- New `components/map/airportPopup.ts` (or inline in `layers.ts`/`MapView.tsx`): formats airport properties (iata_code, icao_code, name, municipality, iso_country) into popup HTML, including flag-emoji-from-ISO-code and country-name-from-ISO-code helpers.
- No new dependencies (`Intl.DisplayNames` and regional-indicator flag emoji are native).
