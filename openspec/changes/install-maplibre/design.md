...
- **"Pilot map" mode overlays real FAA VFR sectional chart tiles from ChartBundle** (`https://wms.chartbundle.com/tms/1.0.0/sec/{z}/{x}/{y}.png?origin=nw`, free public XYZ tiles sourced from FAA's public-domain raster charts, no key required) as a raster source/layer toggle, rather than restyling the vector basemap. Enabling pilot mode adds the ChartBundle raster layer above the base style (and can dim/hide base-style layers underneath it) and disabling it removes the raster layer, leaving the current light/dark topographic style; airports and military bases stay on top via the same `addCustomLayers` re-apply pattern used for theme swaps.
...
- **[Risk]** ChartBundle is a free third-party community service, not an official FAA/government-hosted endpoint — it could go down, rate-limit, or change its URL scheme without notice. **Mitigation**: pilot mode fails gracefully (raster layer just doesn't load) rather than breaking the rest of the map; the base topographic map remains fully functional either way.
...
- **Pilot map mode**: real FAA VFR sectional chart tiles via ChartBundle's free public XYZ endpoint, not a restyled vector basemap.
