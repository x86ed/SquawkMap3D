...
- [x] 6.3 Toggle the theme (light/dark) after location is known: confirm the dish and rings remain visible, correctly positioned, after the style reload. (Verified in browser: all 3 labeled rings persist correctly through a light/dark switch. A one-time, non-fatal MapLibre-internal WebGL warning — `Cannot read properties of undefined (reading 'shaderPreludeCode')`, inside MapLibre's own terrain-depth render pass, not this app's code — appeared in the console during the transition; map remained fully interactive and rendered correctly afterward. Noted for awareness, not blocking.)
