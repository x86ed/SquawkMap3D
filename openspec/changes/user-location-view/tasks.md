...
- [x] 6.1 With geolocation permission granted: confirm on initial load the dish marker and 4 labeled rings (50/100/150/200 NM) appear centered on the resolved location, and the "My location" button re-flies/re-centers on click. (Verified in browser with a stubbed geolocation position: all 4 rings render labeled and centered immediately; dish renders correctly but is only visually distinguishable once zoomed in close, since it's a small marker next to a 200 NM ring — see design.md addendum.)
