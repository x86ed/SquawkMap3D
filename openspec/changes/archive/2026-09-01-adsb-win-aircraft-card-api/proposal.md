...
- Restores `xpProgressToNextTier`/"N% to next tier" progress bar, driven by a **confirmed**, tester-sourced default per-tier XP threshold table (`components/map/overlay/tierProgress.ts`) — adsb.win's API documents no per-tier threshold itself, but the tester has supplied the real values directly (Alloy 0 / Carbon 25,000 / Titanium 100,000 / Iridium 300,000 / Plasma 1,000,000 / Quantum 3,000,000 XP), clamped to never claim 100% ahead of the API's own `tier` field, and isolated in its own module for a cheap update if adsb.win adds a tier beyond `quantum`. The `tier` field itself (e.g. "Alloy") is adsb.win's own distinct *material*-tier ladder — unrelated to and not replacing this app's existing *rarity*-tier ladder (`aircraft-rarity` capability, Decision 5), which stays wired to its own local vendored dataset, untouched by this change. See design.md Decision 4/4a.
...
- `components/map/overlay/tierProgress.ts` (new): `computeTierProgress(tierName, xp)` plus the confirmed default per-tier XP threshold table, isolated in its own module so it can be updated (e.g. if adsb.win adds a new tier) without touching `aircraftModelCard.ts`.
