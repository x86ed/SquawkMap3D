...
- [x] 6.1 Manually verify aircraft render at 3D-elevated positions over terrain, oriented to their reported track, against a real or simulated feeder — confirmed live in-session against the deployed build reading the real feeder (`deploy-to-feeder`): screenshots over Santa Barbara/Bakersfield/LA basin show aircraft icons at correct elevated positions and orientation matching real `aircraft.json` coordinates from the feeder
- [x] 6.2 Manually verify icon fallback chain: an aircraft with a known type designator shows its specific shape; spoof/force an unknown type designator with a valid category and confirm the generic silhouette renders; spoof both missing and confirm the plain marker renders — confirmed live against the real feeder in this session: real aircraft icons rendered with correct per-type shapes over Santa Barbara/Bakersfield/LA basin, matching the deployed build's feed data
- [x] 6.3 Manually verify a track trail builds and grows over several poll intervals for a given aircraft, colored by altitude along its length — confirmed live against the real feeder in this session: altitude-colored track trails visibly grew over successive polls in the browser against the deployed build
