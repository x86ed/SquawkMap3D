...
- Add 4 concentric, labeled range rings around the user's location at 50 NM, 100 NM, 150 NM, and 200 NM (great-circle radius), each labeled with its distance.
...
### New Capabilities
- `user-location-marker`: on-demand jump-to-location control, MapLibre-built-in pan/tilt/zoom controls, a 3D dish marker at the user's location, and labeled 50/100/150/200 NM range rings centered on that location.
...
- `components/map/layers.ts` (or a new `components/map/userLocation.ts`): add source/layers for the dish extrusion and the 4 ring circles + labels.
